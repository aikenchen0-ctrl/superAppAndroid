...
final class BlinkCaptureViewController: UIViewController {
    ...
    private var faceFramesSeen = 0
    private var sessionStartMs: Int64 = 0
    private var lastStatusUpdateMs: Int64 = 0
    ...
    private func configureAndStartSession() {
        sessionQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                self?.statusLabel?.text = "SDK 已启动，正在等待相机帧\n请正对前置摄像头"
            }
        }
    }
    ...
    private func handleFrameResult(_ frameResult: BlinkFrameResult) {
        ...
        updateRuntimeStatus(frameResult: frameResult, event: event, timestampMs: timestampMs)
    ...
    private func updateRuntimeStatus(
        frameResult: BlinkFrameResult,
        event: BlinkCaptureResult?,
        timestampMs: Int64
    ) {
        guard event == nil, timestampMs - lastStatusUpdateMs >= 300 else { return }
        lastStatusUpdateMs = timestampMs

        let elapsedMs = sessionStartMs > 0 ? max(0, timestampMs - sessionStartMs) : 0
        let text: String
        if frameResult.hasFace {
            text = String(
                format: "SDK 识别中，尚未返回事件\n已检测到人脸：%d/%d 帧\nleftEAR %.3f｜rightEAR %.3f\n已运行 %.1f 秒",
                faceFramesSeen,
                framesSeen,
                frameResult.leftEar,
                frameResult.rightEar,
                Double(elapsedMs) / 1000.0
            )
        } else {
            text = String(
                format: "SDK 识别中，尚未检测到人脸\n相机帧：%d\n请正对前置摄像头并保持光线充足\n已运行 %.1f 秒",
                framesSeen,
                Double(elapsedMs) / 1000.0
            )
        }
