// Adapted from Shizuku's manager/src/main/jni/starter.cpp.
// Original purpose: launch a long-living Java process (the "server") in the
// ADB shell context (uid 2000) by exec-ing /system/bin/app_process.
//
// Differences from upstream:
//   - SERVER_NAME / SERVER_CLASS_PATH point to AdbCoreServerMain instead of
//     Shizuku's ShizukuService.
//   - The "-Dshizuku.library.path" system property is renamed to
//     "-Dadbcore.library.path" to avoid namespace clashes if a host app
//     happens to also embed Shizuku.
//   - Removed the SU/root flow because adbcore is ADB-only.
//
// Built into "libadbcore.so" — note the `.so` suffix is a packaging trick:
// Android extracts everything under lib/<abi>/ on install and preserves
// executable permission, allowing the ADB shell user to exec this file.
...
#define SERVER_NAME "adbcore_server"
#define SERVER_CLASS_PATH "com.adbcore.server.AdbCoreServerMain"
...
static void run_server(const char *dex_path, const char *main_class, const char *process_name,
                       const char *host_package, const char *host_authority) {
...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    ARG_PUSH_FMT(argv, "-Dadbcore.library.path=%s", lib_path)
    ARG_PUSH_FMT(argv, "-Dadbcore.host.package=%s", host_package ? host_package : "")
    ARG_PUSH_FMT(argv, "-Dadbcore.host.authority=%s", host_authority ? host_authority : "")
...
static void start_server(const char *path, const char *main_class, const char *process_name,
    ...
    switch (pid) {
        ...
        case 0: {
            ...
            run_server(path, main_class, process_name, host_package, host_authority);
        }
        default: {
            printf("info: adbcore_server pid is %d\n", pid);
            printf("info: adbcore_starter exit with 0\n");
            exit(EXIT_SUCCESS);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc; ++i) {
        if (strncmp(argv[i], "--apk=", 6) == 0) {
            ...
            host_package = argv[i] + 10;
        } else if (strncmp(argv[i], "--authority=", 12) == 0) {
            host_authority = argv[i] + 12;
        }
    }

    uid_t uid = getuid();
    if (uid != 0 && uid != 2000) {
        perrorf("fatal: run adbcore_starter from non root nor adb user (uid=%d).\n", uid);
        exit(EXIT_FATAL_UID);
    }
    ...
    foreach_proc([](pid_t pid) {
        ...
        if (kill(pid, SIGKILL) == 0)
            ...
            perrorf("fatal: can't kill %d, please try to stop existing adbcore_server first.\n", pid);
    ...
    if (apk_path.empty()) {
        perrorf("fatal: --apk=<path> not provided\n");
    ...
    printf("info: starting server (package=%s, authority=%s)...\n",
           host_package.c_str(), host_authority.c_str());
    fflush(stdout);
    LOGD("start_server");
