...
- 侧边移动过程中只产生候选和预览，只有合法 `UP` 才执行一次动作；底部点击仍在 `UP` 确认，但方向手势越过稳定距离阈值、长按达到时间阈值或上滑停住达到保持阈值时立即提交一次，后续 `UP/CANCEL` 只负责收尾。
...
- `DOWN` 建立事务但不执行。
- 底部 Tap 在 `UP` 执行一次；方向和计时类手势在 `MOVE`/计时器达到不可逆阈值时执行一次。
- 普通上滑默认在越过稳定距离阈值时执行；“上滑停住”与普通上滑共享前缀，默认不绑定，用户显式绑定后才进入消歧规则。
- 提前提交后的 `UP/CANCEL` 只做清理，不重复执行；尚未提交的事务由 `CANCEL`、多指、失焦和回缩取消。
- `CANCEL`、多指、失焦和回缩取消。
