...
public final class AispectImpactCNNClassifier {
    ...
    private Model loadModel(ModelInfo info) {
        ...
        try {
            ...
            // 因果时间网格已经在运行时生成固定数组，数组下标从 0 开始；
            // scaler 中的 -3...+5 是逻辑窗口坐标，不能直接作为数组下标。
            if (AispectCausalPressFeatureBuilder.isTimeGridFeatureContract(info.featureContract)
                    && containsNegative(frameIndices)) {
                frameIndices = sequentialFrameIndices(frameIndices.length);
            }
    ...
    private ModelInfo catalogModelInfo(
        ...
        try {
            ...
            // 时间网格模型的旧 scaler 只声明 gridOffsetsMs；从最大正端点恢复按下期合同。
    ...
    private static boolean containsNegative(int[] values) {
        if (values == null) {
            return false;
        }
