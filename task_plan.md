...
## 本轮：群聊“聊天信息”全屏悬浮界面（2026-08-18）

### 目标

按确认规格完成 30dp 状态区、M3 toolbar、5 列成员网格、群资料与开关设置、成员搜索/详情、二维码保存、输入编辑和退出群聊，并复用现有 SCRM 真实接口与根级进出场动画。

### 阶段

- [x] 发现：确认现有 `GroupInfoHost/Screen`、共享全屏工具栏、根级动画及群管理接口。
- [ ] 测试：补充页面结构、导航、5 列网格和二维码 payload 契约。
- [ ] 实现：重构主页面并补齐搜索、二维码和输入子流程。
- [ ] 验证：运行定向测试、Kotlin 编译和 `git diff --check`。

### 约束

- 不回退当前 `com/paifa/univerge` 目录迁移或其他已有改动。
- 不新增静默 fallback、假成功或吞错；远端写操作继续等待真实任务结果。
- “显示群成员昵称”无后端专用接口，保留为明确的本地显示偏好；其余远端开关调用既有 SCRM API。
