/*
 * 功能概览：定义所有可识别的边缘手势及其持久化 ID。
 * Kotlin 语法提示：枚举可以带构造参数；`entries.firstOrNull` 会返回首个匹配项或 null。
 */
