...
class BackWaveOverlayControllerTest {
    @Test
    fun sidePanelLabelsKeepFixedItemsBeforeCustomItems() {
        assertEquals(
            listOf("关闭所有", "返回", "主页"),
            sideFunctionPanelLabels(listOf("home")) { id ->
                if (id == "home") "主页" else error("unexpected action: $id")
            }
        )
    }
    ...
    fun gestureSurfaceUsesWindowHardwareAccelerationWithoutDedicatedLayer() {
        val method = runCatching {
            Class.forName("com.paifa.univerge.accessibility.BackWaveOverlayControllerKt")
    ...
    fun gestureSurfaceWindowIsLocalizedAroundTheTouchStart() {
        val fullScreen = BackWaveAnchor(side = EdgeSide.LEFT, edgeX = 0, y = 0, height = 2400)
    ...
    @Test
    fun edgeWaveAnchorUsesTheActualInsetBoundaryInsteadOfThePhysicalScreenEdge() {
        val leftAnchor = BackWaveAnchor(
            side = EdgeSide.LEFT,
            edgeX = 48,
            y = 0,
            height = 2400
        )
        val rightAnchor = BackWaveAnchor(
            side = EdgeSide.RIGHT,
            edgeX = 1032,
            y = 0,
            height = 2400
        ...
        assertEquals(48, backWaveWindowX(leftAnchor, waveWidthPx = 184, screenWidthPx = 1080))
        assertEquals(848, backWaveWindowX(rightAnchor, waveWidthPx = 184, screenWidthPx = 1080))
    }
