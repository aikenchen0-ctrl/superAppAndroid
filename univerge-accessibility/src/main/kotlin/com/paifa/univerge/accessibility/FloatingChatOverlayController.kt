...
import com.paifa.univerge.accessibility.data.FloatingChatMessageStore
...
import com.paifa.univerge.accessibility.data.ScrmOperationStore
import com.paifa.univerge.accessibility.data.localThreadIdForSelection
import com.paifa.univerge.accessibility.data.toLocalChatMessage
import com.paifa.univerge.accessibility.floatingchat.media.mediaPreviewCoversSystemBars
import com.paifa.univerge.accessibility.floatingchat.chat.toChatThreadSelection
import com.paifa.univerge.accessibility.floatingchat.chat.toPrototypeToolSelection
import com.paifa.univerge.accessibility.floatingchat.chat.navigationStateAfterOutgoingMessage
import com.paifa.univerge.accessibility.floatingchat.moments.toAppMomentPost
import com.paifa.univerge.accessibility.floatingchat.moments.toLocalMomentPost
...
import com.paifa.univerge.accessibility.scrm.ScrmChatRoomQuery
import com.paifa.univerge.accessibility.scrm.ScrmChatRoomMemberQuery
import com.paifa.univerge.accessibility.scrm.ScrmContactQuery
import com.paifa.univerge.accessibility.scrm.ScrmOperationProcessorResult
import com.paifa.univerge.accessibility.scrm.ScrmOperationRunner
import com.paifa.univerge.accessibility.scrm.ScrmSelectedSession
import com.paifa.univerge.accessibility.scrm.ScrmSettingsManager
import com.paifa.univerge.accessibility.scrm.ScrmTextMessageRoute
import com.paifa.univerge.accessibility.scrm.ScrmFloatingAccountConversation
...
import com.paifa.univerge.accessibility.scrm.ScrmChatRoom
import com.paifa.univerge.accessibility.scrm.ScrmChatRoomMember
import com.paifa.univerge.accessibility.scrm.ScrmChatMessage
import com.paifa.univerge.accessibility.scrm.ScrmDevice
import com.paifa.univerge.accessibility.scrm.ScrmMessagePreflightFailure
import com.paifa.univerge.accessibility.scrm.ScrmWechatAccount
import com.paifa.univerge.accessibility.scrm.scrmFloatingChatConversation
import com.paifa.univerge.accessibility.scrm.scrmFloatingAccountRouteForSelection
import com.paifa.univerge.accessibility.scrm.scrmMessagePreflightFailure
import com.paifa.univerge.accessibility.scrm.scrmMessageOperationType
import com.paifa.univerge.accessibility.scrm.scrmOutboxItemForMessage
import com.paifa.univerge.accessibility.scrm.scrmTextRouteForMessageThread
import com.paifa.univerge.accessibility.scrm.withScrmFailureState
import com.paifa.univerge.accessibility.scrm.withScrmQueueState
...
internal class FloatingChatOverlayController(
    ...
    fun refreshEdgeGestureConfig() {
        if (state == FloatingChatOverlayState.Expanded && composeView != null) {
            edgeGestureConfigRevision += 1
            applySystemGestureExclusionToExpandedView()
        }
    }
    ...
    private fun openExternalDocument(message: FloatingChatMessage): Boolean {
        ...
        val intent = Intent()
            .setClassName(
                context.packageName,
                "com.paifa.univerge.app.FloatingChatDocumentViewerActivity"
            )
    ...
    private fun defaultToolThreadSelection(
        conversation: com.paifa.univerge.core.model.FloatingChatConversation
    ...
    fun sendFavoriteCollectionItem(
        ...
        if (
            item.accountId != selectedAccountId &&
                item.accountId != com.paifa.univerge.accessibility.floatingchat.tools.LegacyFavoriteAccountId
        ) {
            return false
        }
        localMessageSequence += 1
        ...
        val message = prepareOutgoingMessageForScrm(
            com.paifa.univerge.accessibility.floatingchat.tools.favoriteCollectionPreviewMessage(item).copy(
    ...
    private fun showState(nextState: FloatingChatOverlayState, force: Boolean = false) {
        ...
        val view = ComposeView(context).apply {
            ...
            setContent {
                when (nextState) {
                    ...
                    FloatingChatOverlayState.Expanded -> {
                        val edgeConfigRevision = edgeGestureConfigRevision
                        val leftEdgeConfigs = remember(edgeConfigRevision) {
                            preferences.edgeConfigs(EdgeSide.LEFT)
                        }
                        val rightEdgeConfigs = remember(edgeConfigRevision) {
                            preferences.edgeConfigs(EdgeSide.RIGHT)
                        }
                        FloatingChatOverlay(
                        conversation = conversation,
        ...
        // can race the addView call and leave the chat action with no visible view.
        if (shouldCommitFloatingChatExpansion(
                requestedExpanded = nextState == FloatingChatOverlayState.Expanded,
                mounted = mounted
            ) || nextState == FloatingChatOverlayState.Collapsed
        ) {
            updateOverlayState(nextState)
        }
        applySystemGestureExclusionToExpandedView()
    ...
    private fun applySystemGestureExclusionToExpandedView() {
    ...
    ): com.paifa.univerge.core.model.FloatingChatConversation {
        return try {
    ...
    private fun loadNextScrmPrefetchConversationFromApi(): com.paifa.univerge.core.model.FloatingChatConversation? {
    ...
    private fun loadScrmConversationForSession(
        ...
        loadedAccountConversations.forEach { accountConversation ->
            ...
            Log.i(
                ...
                "api conversation route=${accountConversation.weChatId.takeLast(4)} contacts=${accountConversation.contacts.size} contactAvatars=${accountConversation.contacts.count { !it.displayAvatarUrl.isNullOrBlank() }} " +
                    "groups=${accountConversation.chatRooms.size} groupAvatars=${accountConversation.chatRooms.count { !it.displayAvatarUrl.isNullOrBlank() }} " +
                    "memberAvatars=${accountConversation.chatRoomMembers.values.flatten().count { !it.displayAvatarUrl.isNullOrBlank() }}"
            )
        }
        val accountConversations = mergeScrmAccountConversationCache(
            cachedConversations = cachedScrmAccountConversations.values,
        ...
        val nextConversation = scrmFloatingChatConversation(
            base = FloatingChatPrototype.sampleConversation(),
            contacts = emptyList(),
            accountConversations = accountConversations,
            accounts = accounts,
        ...
        Log.i(
            AVATAR_DIAGNOSTICS_TAG,
            "mapped conversation contacts=${nextConversation.contacts.size} contactAvatars=${nextConversation.contacts.count { !it.avatarUrl.isNullOrBlank() }} " +
                "groups=${nextConversation.groupContacts.size} groupAvatars=${nextConversation.groupContacts.count { !it.avatarUrl.isNullOrBlank() }} " +
    ...
    private fun applyScrmConversation(
        nextConversation: com.paifa.univerge.core.model.FloatingChatConversation
    ) {
...
private fun floatingChatConversationDataSource(
    value: com.paifa.univerge.core.model.FloatingChatConversation
...
internal fun shouldCommitFloatingChatExpansion(
    requestedExpanded: Boolean,
    mounted: Boolean
): Boolean = requestedExpanded && mounted
