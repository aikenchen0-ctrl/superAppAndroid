...
internal data class FavoriteCollectionItem(
    val accountId: String = LegacyFavoriteAccountId,
...
internal fun favoriteCollectionItems(
    ...
    accountId: String = LegacyFavoriteAccountId
    ...
    val currentAccountStoredItems = favoriteCollectionItemsForAccount(refreshedStoredItems, accountId)
    val explicitFavorites = messages.filter { message ->
        favoriteMessageIds[message.id] == true || favoriteMediaIds[message.id] == true
    }.map { message -> message.toFavoriteCollectionItem(accountId) }
    val seededFavorites = if (currentAccountStoredItems.isEmpty() && explicitFavorites.isEmpty()) {
        ...
        }.take(6).map { message -> message.toFavoriteCollectionItem(accountId) }
    } else {
        emptyList()
    }
    return favoriteCollectionItemsForAccount(
        mergeFavoriteCollectionItems(refreshedStoredItems + explicitFavorites + seededFavorites),
        accountId
    )
}

internal fun favoriteCollectionItemsForAccount(
    items: List<FavoriteCollectionItem>,
    accountId: String
): List<FavoriteCollectionItem> = items.filter { item -> item.accountId == accountId }
...
internal fun refreshFavoriteCollectionSourcesFromMessages(
    ...
    return items.map { item ->
        if (accountId != null && item.accountId != accountId) return@map item
        val currentMessage = messagesById[item.messageId] ?: return@map item
        item.copy(source = currentMessage.toFavoriteCollectionItem(item.accountId).source)
    }
}
...
internal fun updateFavoriteCollectionItems(
    ...
    val nextItems = if (favorite) {
        mergeFavoriteCollectionItems(listOf(message.toFavoriteCollectionItem(accountId)) + items)
            .take(FavoriteCollectionMaxCount)
    } else {
        items.filterNot { item -> item.accountId == accountId && item.messageId == message.id }
    }
    items.clear()
...
internal fun mergeFavoriteCollectionItems(
    ...
    return items.filter { item -> seen.add("${item.accountId}\u001F${item.messageId}") }
}
...
internal fun favoriteCollectionItemsAfterRemove(
    ...
    return mergeFavoriteCollectionItems(
        items.filterNot { item ->
            item.accountId == removedItem.accountId && item.messageId == removedItem.messageId
        }
    ).take(FavoriteCollectionMaxCount)
}
...
internal fun favoriteCollectionSerializationRoundTrips(): Boolean {
    ...
    return decoded?.accountId == item.accountId &&
        decoded.messageId == item.messageId &&
...
private fun FloatingChatMessage.toFavoriteCollectionItem(accountId: String): FavoriteCollectionItem {
    val title = when (type) {
...
private fun serializeFavoriteCollectionItems(items: List<FavoriteCollectionItem>): String {
    ...
    cappedItems.forEachIndexed { index, item ->
        ...
        properties.setProperty(prefix + "accountId", item.accountId)
            ...
            for (index in 0 until count) {
                ...
                add(
                    FavoriteCollectionItem(
                        accountId = properties.getProperty(prefix + "accountId")
                            ?.takeIf { value -> value.isNotBlank() }
                            ?: LegacyFavoriteAccountId,
                        messageId = messageId,
                        type = type,
...
internal const val LegacyFavoriteAccountId = "__legacy_favorite_account__"
