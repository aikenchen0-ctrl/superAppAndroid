...
import com.paifa.univerge.accessibility.BuildConfig
...
class ScrmSettingsManager(context: Context) {
    ...
    internal fun loadSelectedSessionOrBootstrap(): ScrmSelectedSession {
        // 先使用用户已保存的 API Key；只有未配置或认证失效时才尝试 Debug 专用的自动引导凭据。
        // 凭据只从 local.properties/环境变量读取，不能写入源码或日志。
...
internal data class ScrmSelectedSession(
    ...
    val openApiRaw: ScrmOpenApiRawApi,
